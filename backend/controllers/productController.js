...
exports.getAllProducts = catchAsyncErrors(async(req, res, next)=>{
    ...
    let apifeature = new Apifeature(Product.find(), req.query).search().filter().pagination(resultPerPage);
    let apifeature1 = new Apifeature(Product.find(), req.query).search().filter();

    let products = await apifeature.query;
    let products1 = await apifeature1.query;
